...
static Sound *bounce_sound = NULL;
static Sound *bounce2_sound = NULL;
 ...
 */
static inline bool is_touching_player(unsigned int _player) {
    return (
 ...
 */
static inline bool is_heading_towards(unsigned int _player){
...
inline void initialize_ball() {
    ...
    ball.reset = reset;
    ball.is_heading_towards = is_heading_towards;
    ball.is_touching_player = is_touching_player;
}
...
inline void bounce() {
    const unsigned int _player = ball.is_heading_towards(PLAYER_ONE) ? PLAYER_ONE : PLAYER_TWO;
...
inline void process_ball(double delta_time) {
    ...
    if(ball.is_heading_towards(PLAYER_ONE)){
        if (ball.is_touching_player(PLAYER_ONE))
            bounce();
    } else {
        if (ball.is_touching_player(PLAYER_TWO))
            bounce();
    }
...
inline void draw_ball() {
    app_draw_rect(&ball.position, &ball.size);

    if (app_debug){
        Color bcol = {1.0, 0.0, 1.0, 1.0};
        app_set_draw_color(&bcol);
        app_draw_line(&(Vector2){0.0, (ball.position.y + ball.size.y/2)},&(Vector2){window_width, (ball.position.y + ball.size.y/2)});
